...
def residual_train():
    ...
    for epoch in range(num_epochs):
        ...
        correct = torch.zeros(train_data_length, dtype=torch.bool)
        ...
        with torch.no_grad():
            for data, target in evaluation_data_loader:
                ...
                temp_correct = pred.eq(target)
                total_correct += temp_correct.sum().item()

                correct[start_index: start_index + evaluation_batch_size] = temp_correct
                start_index += evaluation_batch_size

        for idx in range(train_data_length):
            if not correct[idx].item():
                probs[idx] = beta
...
if __name__ == '__main__':
    for j in range(20):
        ...
        residual_train()
        loc = "./CNN-l2-freeze-prob/more-false-beta-" + str(j) + ".pt"
