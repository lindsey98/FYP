...
idx_to_info_dic = dict()
count_to_idx_dict = dict()
        ...
        for data, target in evaluation_data_loader:
            ...
            for i in range(evaluation_batch_size):
                if correct_list[i].item():
                    internal_signals.append(resize_feature[i])
                    count_to_idx_dict[count] = start_index + i
                    count += 1
                    idx_to_info_dic[start_index + i] = pred[i].item()

            start_index += evaluation_batch_size

    internal_signals = torch.stack(internal_signals, dim=0)
            ...
            for data, target in evaluation_data_loader:
                ...
                for i in range(evaluation_batch_size):
                    if not correct_list[i].item():
                        resize_feature[i].expand(internal_signals.shape)
                        dist = internal_signals.add(
                            -resize_feature[i].expand(internal_signals.shape)).pow(2).sum(dim=1).pow(.5)
                        knn_indices = dist.topk(k, largest=False, sorted=False)

                        correct_predict = 0
                        for m in knn_indices[1]:
                            if idx_to_info_dic[count_to_idx_dict[m.item()]] == target[i].item():
                                correct_predict += 1
                        probs[start_index + i] += 2 ** correct_predict * 0.5 ** (k - correct_predict)
...
# 10, 5, 1, 0.5, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0005


if __name__ == '__main__':
    for j in [1000, 500, 100, 50, 10, 5, 1, 0.5, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0005]:
        alpha = j
        print(alpha, k)
        ref_model.load_state_dict(state_dict)
        model.load_state_dict(state_dict)
        residual_train()
        loc = "./CNN-l2-freeze-knn/new-k-" + str(k) + "-alpha-" + str(alpha) + ".pt"
