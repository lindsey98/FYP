...
for data, target in train_data_loader:
    ...
    loss_value = list()
    for i in [0, 1, 2]:
        resize_feature = features_before[i].view(features_before[i].size(0), features_before[i].size(1), -1).mean(
            axis=2)
        resize_ref_feature = features_after[i].view(features_after[i].size(0), features_after[i].size(1), -1).mean(
            axis=2)
    ...
    f = open("./regression_for_first_1000_CNN.csv", "w")
