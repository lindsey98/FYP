...
class ModelOutputsForNewSiamese():
    """ Class for making a forward pass, and getting:
    1. The network output.
    2. Activations from intermeddiate targetted layers.
    3. Gradients from intermeddiate targetted layers. """
    ...
    def __call__(self, x):
        target_activations = []
        x = self.model.root(x)
        for name, module in self.model.body._modules.items():
            if module == self.feature_module:
                target_activations, x = self.feature_extractor(x)
            elif "avgpool" in name.lower():
                x = module(x)
        ...
        x = self.model.head.gn(x)
        x = self.model.head.relu(x)
        x = self.model.head.avg(x)
        x = l2_norm(x)
        return target_activations, x


class GradCamForNewSiamese:
    def __init__(self, model, feature_module, target_layer_names):
        self.model = model.to(device)
        self.feature_module = feature_module
        self.model.eval()

        self.extractor = ModelOutputsForNewSiamese(self.model, self.feature_module, target_layer_names)
